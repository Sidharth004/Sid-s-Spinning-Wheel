...
interface ModalProps {
  ...
  onClose: () => void;
  title: string;
  content: string | Experience[] | Contact | WritingContent | AboutContent| SideQuest[] | GrowthJam[] | null;
...
const LabelBadge = ({ type }: { type: string }) => {
  const colors: Record<string, string> = {
    ...
    'Hot Take':'bg-yellow-100 text-yellow-800'
  };
...
export function Modal({ isOpen, onClose, title, content, type = 'default' }: ModalProps) {
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-[800px] bg-white/95 backdrop-blur-sm max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex justify-center">
            <div className="w-40 h-40 rounded-full overflow-hidden ring-4 ring-gray-200">
              <img
  ...
  if (type === 'growthJams') {
    const portfolioItems = content as GrowthJam[];
    return (
      <GrowthJamsModal
        isOpen={isOpen}
        onClose={onClose}
