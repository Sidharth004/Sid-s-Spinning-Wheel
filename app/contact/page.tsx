  ...
  return (
    <main className="min-h-screen bg-[#121210] text-white px-6 py-12">
      <div className="max-w-xl mx-auto">
        ...
        <p className="text-gray-400 mb-12">Find me anywhere below.</p>

        <div className="space-y-3">
          {Object.entries(contact).map(([platform, info]) => (
            <a
              key={platform}
              href={info.url}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-4 p-4 border border-gray-800 rounded-xl hover:border-gray-600 transition-colors group"
            >
              <span className="text-gray-500 group-hover:text-white transition-colors">
                {platformIcons[platform]}
              </span>
              <span className="font-semibold text-white flex-1">{platform}</span>
